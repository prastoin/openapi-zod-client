      ...
      const endpoints = makeApi([
        {
          ...
          requestFormat: "json",
          response: z.array(
            z.union([FooBar.and(Bar).and(Foo), z.union([FooBar, Bar, Foo])])
          ),
        },
      ]);
...
test("Import only word matching common model from zod expression", async () => {
    const openApiDoc: OpenAPIObject = {
        openapi: "3.0.0",
        info: { title: "Foo bar api", version: "1.0.1" },
        paths: {
            "/bar": {
                ...
                post: {
                    summary: "bar",
                    description: "Bar",
                    tags: ["should-only-import-foobar"],
                    responses: {
                        "200": {
                            description: "Success",
                            content: {
    ...
    expect(output).toMatchInlineSnapshot(`
      ...
      export { Should_only_import_foobarApi } from "./should_only_import_foobar";
      ",
          "controller_bar": "import { makeApi, Zodios, type ZodiosOptions } from "@zodios/core";
      import { z } from "zod";
      ...
      const Foo = z.object({ foo: z.boolean() }).partial().passthrough();
      ...
      ",
          "should_only_import_foobar": "import { makeApi, Zodios, type ZodiosOptions } from "@zodios/core";
      import { z } from "zod";

      import { FooBar } from "./common";

      const BarFoo = z
        .object({ foo: z.string(), bar: z.boolean() })
        .partial()
        .passthrough();
      ...
      const endpoints = makeApi([
        {
          ...
          description: \`Bar\`,
          requestFormat: "json",
          response: z.union([FooBar, BarFoo]),
        },
      ]);

      export const Should_only_import_foobarApi = new Zodios(endpoints);
